...
Existing flood screening systems report aggregate accuracy or ROC-AUC, neither of which
surfaces category-level false positive rates critical for the recall-first screening objective.
...
\item Recall-first evaluation using PR-AUC alongside per-category FP rates with Clopper-Pearson CIs reveals per-threshold operating-point differences invisible in aggregate accuracy.
...
neither of which surfaces per-threshold operating-point differences between architectures
nor per-category false positive rates on confounding categories.
For a recall-first screening objective, \PR{} and per-category FP analysis are the
appropriate diagnostic tools \cite{Davis2006,Saito2015}.
We demonstrate this empirically in \cref{sec:results:baseline}: despite comparable
...
\textbf{Contributions.}
\begin{enumerate}
  \item \textbf{Recall-first evaluation with per-category FP analysis.}
        We report \PR{} as the primary metric and provide per-category FP rates
        with \CP{} 95\,\% confidence intervals on 4,099 deduplicated street-level
        images across eight fine-grained categories, demonstrating that aggregate
        metrics (accuracy, ROC-AUC) do not surface per-threshold operating-point
        differences relevant to the recall-first screening objective.
...
and \citet{Shrivastava2016} systematised online hard example mining (OHEM).
...
\citet{Davis2006} showed that PR-AUC better reflects recall-oriented performance when the positive class is rare.
\citet{Saito2015} empirically confirmed its advantages in class-imbalanced screening regimes.
...
\begin{figure}
  ...
  \node[datast, right=0.3cm of data] (split) {\textbf{Split}\\[2pt]
    \scriptsize 80/20\\ stratified\\ seed\,=\,42};
  ...
  \node[hnmst, below=0.7cm of conf] (hnm) {\textbf{HNM Pipeline}\\[2pt]
    \scriptsize rank by $\hat{p}$ from $M_1$\\ top $\pi{=}20\%$\\ $5{\times}$ augment};
...
All models are trained with binary cross-entropy (BCE, Eq.~\ref{eq:bce}) combined with
balanced class weights to correct for class frequency.
\begin{equation}
  \mathcal{L}_\text{BCE} = -w_y \bigl[y \log\hat{p} + (1-y)\log(1-\hat{p})\bigr]
  \label{eq:bce}
\end{equation}
where $w_y$ is the inverse-frequency class weight for label $y$.
...
the top $\pi{=}20\,\%$ are selected.
Percentile-based selection is calibration-invariant, ensuring consistent candidate set sizes
across architectures.
A partition-safety check verifies that no selected hard negative appears in the validation set, preventing data leakage.
...
If HNM outperforms extended training, additional compute alone cannot explain the gain;
if HNM also outperforms random injection, difficulty ranking rather than mere category exposure is the active ingredient.

\subsection{Evaluation protocol}
\label{sec:methods:eval}

\PR{} is the primary metric (motivation in \cref{sec:results:baseline}).
...
\begin{table}
  ...
  \begin{tabular}{llrrrrrrrr}
    ...
    \ResNet{}  & BCE   & \textbf{98.5} & 97.2 & \textbf{0.991} & 0.981 &  9 &  3 & \textbf{0.9991} & \textbf{0.9995} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\Cref{tab:confounder} and \cref{fig:confounder_heatmap} report per-category FP rates for both BCE baselines.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{necessary_diagrams/06_confounder_fp_heatmap}
  \caption{Per-category false positive rate heatmap for both BCE baselines
           (val set, seed\,42, $\tau{=}0.5$).
           River is the only category with non-zero FPs in either architecture;
           \EffNet{} BCE river FP rate (9.2\,\%) exceeds \ResNet{} BCE (3.9\,\%),
           identifying it as the primary target for \PHNM{} (\cref{tab:confounder}).}
  \label{fig:confounder_heatmap}
\end{figure}
...
Accuracy is directionally consistent across all conditions: HNM ($98.78\,\%$), extended training ($98.66\,\%$),
random injection ($98.41\,\%$), and baseline ($98.29\,\%$); this ordering is consistent with hierarchical isolation
of the difficulty-ranking contribution, though differences do not reach McNemar significance (see below).
...
\begin{figure}
  ...
  \caption{PR and ROC curves for the four \EffNet{} BCE ablation conditions
           (val set, seed\,42).
           HNM maintains the highest PR-AUC; accuracy decreases in the order
           HNM, extended training, random injection, baseline across the full recall range.
...
\subsection{Recall-first evaluation: why per-category FP analysis matters}
...
Per-category FP analysis (\cref{tab:confounder}) provides the operationally relevant diagnostic:
\EffNet{}'s 9.2\,\% river FP rate identifies the specific weakness \PHNM{} targets,
and the recall difference (7\,FN vs.\ 9\,FN) directly quantifies the cost for a recall-first deployment.
We recommend all flood screening evaluations report \PR{} as the primary metric alongside
per-category FP rates with \CP{} CIs, since aggregate accuracy alone does not characterise
per-threshold operating-point behaviour on imbalanced categories.
...
The directionally consistent ordering---HNM, then extended training, then random injection---is consistent with difficulty ranking as the active contribution, though the differences do not reach statistical significance at this validation set size.
...
\begin{enumerate}
  ...
  \item \textbf{Use BCE + balanced class weights} as the default loss.
